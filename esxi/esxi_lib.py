...
def download_file(url,directory_path):
    ...
    file_path = os.path.join(directory_path, urlparse.urlparse(url)[2].split('/')[-1])
    print 'Donwloading...'
...
def import_vapp(resource_pool, import_spec, host = None, folder=None, server = None):
    u"""Deploy VM
    ...
    for dev_url in lease.info.deviceUrl:
        ...
        for filename in filenames:
            ...
            if filename.find(format) > 0:
                hostname = urlparse.urlparse(server._proxy.binding.url).hostname
                upload_url = dev_url.url.replace("*", hostname)
                #  ここでfilenameにOVFファイルのディレクトリパスを付与する
                filename = os.path.join(os.path.dirname(ovf_file_path), filename)
